...
// Extract plain text from an uploaded training document. Type is decided
// by file content (magic bytes) where possible, never the client's MIME
// header. Same engine as the SOP import.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    logError('training/import parse', err);
  ...
  try {
    ...
    try {
      ...
      logError('training/import extract', err);
      return res.status(400).json({ error: 'Could not read that document. Is the file valid?' });
    }
    if (text === null) {
      return res.status(400).json({ error: 'Unsupported file type. Upload a .pdf, .txt, .md, or .docx file.' });
    }
    if (!text.trim()) {
      return res.status(400).json({ error: 'No readable text found in that document (is it a scanned image?).' });
    }

    const parsed = parseSopText(text.slice(0, MAX_TEXT_CHARS));
    if (!parsed.title && parsed.steps.length === 0) {
      return res.status(400).json({ error: 'Could not find any step-by-step content in that document.' });
    }

    // Map the generic parsed document onto a training draft
    const draft = {
      title: parsed.title,
      description: parsed.summary,
      // Training only has two categories; keep a detected match, else let the admin pick
      category: parsed.category === 'HVAC' || parsed.category === 'Home Performance' ? parsed.category : '',
      steps: parsed.steps.map(s => ({
        title: s.title,
        body: s.summary && !s.body.includes(s.summary) ? `${s.summary}\n${s.body}`.trim() : s.body,
      })),
    };
